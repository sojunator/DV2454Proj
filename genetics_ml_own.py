...
def mutate(network, maxScore):
    for layer in network[1]:
        layer = layer + random.uniform(-1.0, 1.0)  / (100 + network[0])
    return network
...
def testNetwork(network, nrOfSteps, env, render=False):
...
if __name__ == '__main__':
    ...
    for i in range(generations):
        for network in networks:
            network[0] = testNetwork(network[1], 500, env, False)
        ...
        print("Score of gen {} is {}".format(i, networks[0][0]))
        testNetwork(networks[0][1], 500, env, True)
